    ...
    <div class="card-body">
        <form action="{{ route('students.update', $students->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="name" class="form-control" placeholder="Enter student name" value="{{ old('name', $students->name) }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="country_id" class="form-select" required>
                    ...
                    <option value="{{ $country->id }}" {{ old('country_id', $students->country_id) == $country->id ? 'selected' : '' }}>
                        {{ $country->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select name="district_id" class="form-select" required>
                    ...
                    <option value="{{ $district->id }}" {{ old('district_id', $students->district_id) == $district->id ? 'selected' : '' }}>
                        {{ $district->name }}
                    </option>
                    @endforeach
                </select>
